...
class ControllerCommonFileManager extends Controller
{
    public function index() {
        ...
        if($directories) {
            ...
            $images = array_splice($directories, ($page - 1) * 10, 10);
            
            foreach($images as $image) {
                if(substr(str_replace('\\', '/', realpath($image)), 0, utf8_strlen(DIR_IMAGE . 'catalog')) == str_replace('\\', '/', DIR_IMAGE . 'catalog')) {
                    $name = basename($image);
                    ...
                    $data['directories'][] = array(
                        ...
                        'type'  => 'directory',
                        'href'  => $this->url->link('common/filemanager', 'member_token=' . $this->session->data['member_token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'))) . $url, true),
                        //'time'  => date($this->language->get('datetime_format'), filectime($image))
                    );
                }
            }
        }
        ...
        if($files) {
            # Split the array based on current page number and max number of items per page of 10
            $images = array_splice($files, ($page - 1) * 10, 10 - count($data['images']));
            ...
            foreach($images as $image) {
                if(substr(str_replace('\\', '/', realpath($image)), 0, utf8_strlen(DIR_IMAGE . 'catalog')) == str_replace('\\', '/', DIR_IMAGE . 'catalog')) {
                    ...
                    $data['images'][] = array(
                        ...
                        'name'  => $name,
                        'type'  => utf8_strtoupper(pathinfo($image, PATHINFO_EXTENSION)) . ' ' . ucfirst(filetype($image)),
                        'path'  => utf8_substr($image, utf8_strlen(DIR_IMAGE)),
                        'href'  => HTTP_TEMPLATE . 'image/' . utf8_substr($image, utf8_strlen(DIR_IMAGE)),
                        'time'  => date($this->language->get('datetime_format'), filectime($image))
                    );
                }
            }
        ...
        $data['parent'] = $this->url->link('common/filemanager', 'member_token=' . $this->session->data['member_token'] . $url, true);
        ...
        $data['refresh'] = $this->url->link('common/filemanager', 'member_token=' . $this->session->data['member_token'] . $url, true);
        ...
        $data['pagination'] = $this->load->controller('common/pagination', array(
            ...
            'page'  => $page,
            'limit' => 10,
            'url'   => $this->url->link('common/filemanager', 'member_token=' . $this->session->data['member_token'] . $url . '&page={page}')
        ));
    ...
    public function upload() {
        ...
        if(!$this->member->hasPermission('modify', 'common/filemanager')) {
    ...
    public function delete() {
        ...
        foreach($paths as $path) {
            # Convert any encoded html characters.
