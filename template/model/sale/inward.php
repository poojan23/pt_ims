...
class ModelSaleInward extends Model {
    ...
    public function editInward($inward_id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "inward SET customer_id = '" . (int) $data['customer_id'] . "', `top` = '" . (isset($data['top']) ? (int) $data['top'] : 0) . "', sort_order = '" . (int) $data['sort_order'] . "', status = '" . (int) $data['status'] . "', date_modified = NOW() WHERE category_id = '" . (int) $category_id . "'");
